...
const ContactCard = ({ person }) => (
  ...
  >
    <div className="relative rounded-2xl bg-gradient-to-b from-ecell-secondary/30 to-ecell-dark/80 backdrop-blur-sm border border-ecell-secondary/30 p-4 transform transition-all duration-300 group-hover:scale-105 group-hover:border-ecell-primary">
      <div className="flex flex-col items-center">
        <div className="w-full h-48 rounded-xl overflow-hidden border border-ecell-secondary/50">
          <img src={person.image} alt={person.name} className="w-full h-full object-contain rounded-lg" loading="lazy" />
        </div>
        <div className="text-center mt-4">
          <h3 className="text-lg font-semibold text-ecell-text">{person.name}</h3>
          <p className="text-sm text-ecell-primary">{person.role}</p>

          <div className="flex gap-4 justify-center mt-3">
            <a
              href={`mailto:${person.email}`}
              onClick={(e) => e.stopPropagation()} // Fixes footer redirection issue
              className="text-ecell-text hover:text-ecell-primary transition-colors"
            >
              <EnvelopeIcon className="w-5 h-5" />
            </a>
            <a href={person.linkedin} target="_blank" rel="noopener noreferrer" className="text-ecell-secondary hover:text-ecell-primary">
    ...
    <div className="relative w-full min-h-screen bg-ecell-bg py-16 flex flex-col items-center justify-center">
      {teams.map((team) => (
        <div key={team.title} className="w-full text-center mb-12">
          <motion.h1
            className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-ecell-primary to-ecell-secondary text-center mb-8"
            initial={{ opacity: 0, y: -20 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
          >
